(function () {
    var app = angular.module('reindexingFront', ['ngRoute']);
    ...
    app.controller('ReindexController', ['$scope', '$http', function ($scope, $http) {
        ...
        $scope.submit = function() {
            ...
            $http.post('/' + $scope.oldIndex + '/_reindex/' + $scope.newIndex +"?wait_for_completion=true").success(function(response) {
                $scope.newIndex="";
                $scope.oldIndex="";
                $scope.mapping="";
            });
    ...
    app.controller('MoveAliasController', ['$scope', '$http', function ($scope, $http) {
        $http.get('/_stats').success(function(response) {
            $scope.status = response;
            $scope.aliases = []
        });
        ...
        $scope.submit = function() {
            var moveAlias = '{'+
                ...
                ']' +
                '}';
            $http.post('/_aliases', moveAlias).success(function(response) {
                $scope.from = "";
                $scope.to = "";
                $scope.alias = "";
            });
        ...
        $scope.fromIndexChanged = function(){
            $http.get('/' + $scope.from +"/_aliases").success(function(response) {
    ...
    app.controller('CreateAliasController', ['$scope', '$http', function ($scope, $http) {
        $http.get('/_stats').success(function(response) {
            $scope.status = response;
        });
        angular.isDefined
